...
(function () {
  ...
  function renderArena() {
    ...
    if (!A || !lb) return;
    if (!arenaInited) { selCard = todaySelCard(); arenaInited = true; amScrollPending = true; }   // 首次进擂台:默认今日场 + 触发列表滚动定位
    var en = WC.getLang() === "en";
  ...
  function renderAmList(A, en, res) {
    ...
    if (amScrollPending) {   // 首次默认今日场:把列表滚到选中行(只滚 wc-amlist 容器,不动整页)
      amScrollPending = false;
      var selr = host.querySelector(".wc-amrow.sel");
      if (selr) {
        host.scrollTop += selr.getBoundingClientRect().top - host.getBoundingClientRect().top;
        host.scrollLeft += selr.getBoundingClientRect().left - host.getBoundingClientRect().left;
      }
    }
